...
export const HabitCard: React.FC<HabitCardProps> = ({ habit, selectedDate, onToggle, onDelete }) => {
  ...
  return (
    <div className="group flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-md transition-all duration-200">
      <div className="flex items-center space-x-4">
        <button
          ...
          className={`
            ...
            ${isCompleted 
              ...
              : 'bg-transparent border-slate-300 dark:border-slate-600 text-transparent hover:border-indigo-400 dark:hover:border-indigo-400'}
          `}
        ...
        <div>
          <h3 className={`font-semibold text-slate-800 dark:text-slate-200 ${isCompleted ? 'line-through text-slate-400 dark:text-slate-500' : ''}`}>
            {habit.title}
          </h3>
      ...
      <button
        ...
        className="text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 p-2 rounded-full hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100"
